...
Notes for me: I filtered for only January and July and grouped by year,
month and station id. Then I calculated the average of max temp. I usted
these data manipulation sets to plot the graph.
...
```
    ...
    ## Warning: Removed 7038 rows containing missing values (geom_point).

    ## Warning: Removed 7007 row(s) containing missing values (geom_path).
...
one is above the plot and the other is below the plow respectively.
These points are not connected by the path line. There are also some
values that are far from the general average mean temp. For example, in
the January plot, arounf 1999, the average mean tem was below -10
degrees C with would be considered an outlier. Generally, however, most
of the mean temps lie between 10 and 30 degrees C. There are some
...
than 100 separately by year patchwork one is scatter plot maybe? or
contour or hex plot one is box, violin
