...
These points are not connected by the path line. most of the mean temps
lie between 10 and 30 degrees C. Therefore, there are also some values
that are far from the general average mean temp. For example, in the
January plot, around 1999, the average mean temp was below -10 degrees C
with would be considered an outlier. There are several other instances
like this in both months.
...
ny_noaa %>% 
  ...
  labs(
    title = "Max temperature versus Min temperature",
...
violin_plot = 
  ...
  geom_violin() +
  labs(
    title = "Distribution of snowfall between 0 to 100 mm by year",
    x = "Snowfall (mm)",
    y = "Years"
  )
