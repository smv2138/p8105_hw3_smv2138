...
Notes for me: I filtered for only January and July and grouped by year, month and station id. Then I calculated the average of max temp. I usted these data manipulation sets to plot the graph. 
```{r}
ny_noaa %>% 
...
```
Yes, there are two outliers in the July plot around 2005 and 2007, where one is above the plot and the other is below the plow respectively. These points are not connected by the path line. There are also some values that are far from the general average mean temp. For example, in the January plot, arounf 1999, the average mean temp was below -10 degrees C with would be considered an outlier. Generally, however, most of the mean temps lie between 10 and 30 degrees C. 
...
From the violin plot we can see that a lot of the variability in snowfall happened in 2010. 
